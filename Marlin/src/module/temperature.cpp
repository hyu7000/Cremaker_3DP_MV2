...
uint16_t Temperature::isCoolingFanTesting = false; //cremaker
      ...
      if (ELAPSED(ms, next_temp_ms)) {
        ...
        // Make sure heating is actually working
          ...
          if (BOTH(WATCH_BED, WATCH_HOTENDS) || isbed == DISABLED(WATCH_HOTENDS) || ischamber == DISABLED(WATCH_HOTENDS)) {
            ...
            {
              if(heater_id == H_BED)
                _temp_error(heater_id, str_t_thermal_runaway, GET_TEXT(MSG_THERMAL_RUNAWAY_BED));
              else
...
#define _EFANOVERLAP(A,B) _FANOVERLAP(E##A,B)
  ...
  void Temperature::checkExtruderAutoFans() {
    ...
    static const uint8_t fanBit[] PROGMEM = {
      ...
      #elif ENABLED(CREMAKER_CHAMBER) && DISABLED(HAS_MULTI_HOTEND)
        , 1
      #endif
    };
    ...
    #elif ENABLED(CREMAKER_CHAMBER)
      if (degBed() >= CHAMBER_AUTO_FAN_TEMPERATURE || degHotend(-1) >= EXTRUDER_AUTO_FAN_TEMPERATURE)
      {
        SBI(fanState, pgm_read_byte(&fanBit[CHAMBER_FAN_INDEX]));
      }
    #endif
    ...
    LOOP_L_N(f, COUNT(fanBit)) {
      ...
      switch (f) {
        #if HAS_AUTO_FAN_0
          case 0: 
            if(!thermalManager.isCoolingFanTesting)
            {
              _UPDATE_AUTO_FAN(E0, fan_on, EXTRUDER_AUTO_FAN_SPEED); 
            }            
            break;
        #endif
        #if HAS_AUTO_FAN_1
