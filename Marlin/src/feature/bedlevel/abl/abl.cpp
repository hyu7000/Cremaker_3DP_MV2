...
bed_mesh_t z_values;
bed_mesh_t added_z_value; //cremaker
bool isUpdated_Z_Value = false; //cremaker z cal test
...
void print_bilinear_leveling_grid_without_Z_add_value() {
  SERIAL_ECHOLNPGM("Bilinear Leveling Grid_without_Z_add_value:");
  print_2d_array(GRID_MAX_POINTS_X, GRID_MAX_POINTS_Y, 3,
    [](const uint8_t ix, const uint8_t iy) { return z_values[ix][iy] - added_z_value[ix][iy];}
  );
}
...
float bilinear_z_offset(const xy_pos_t &raw) {
  ...
  //PORT_REDIRECT(SerialMask::All); //cremaker zcal test
  ...
  if (prev.y != rel.y || lastg.x != thisg.x || isUpdated_Z_Value) { //cremaker zcal test
    ...
    if ((lastg != thisg) || isUpdated_Z_Value) { //cremaker zcal test
      lastg = thisg;
      ...
      //SERIAL_ECHOLNPAIR("z1:",z1,"d2:",d2,"z3:",z3,"d4:",d4); //cremaker zcal test
    }
    else //cremaker zcal test
    {
      //SERIAL_ECHOLNPAIR("lastg.x:",lastg.x,"thisg.x:",thisg.x); 
      //SERIAL_ECHOLNPAIR("lastg.y:",lastg.y,"thisg.y:",thisg.y); 
    }
  ...
  {
    //SERIAL_ECHOLNPAIR("prev.x:",prev.x,"rel.x:",rel.x);
    //SERIAL_ECHOLNPAIR("prev.y:",prev.y,"rel.y:",rel.y);
  }
  ...
  //cremaker zcal test
  // SERIAL_ECHOLNPAIR("L:",L, " D:", D, " ratio.x", ratio.x); 
