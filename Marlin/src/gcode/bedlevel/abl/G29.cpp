      ...
      for (PR_OUTER_VAR = 0; PR_OUTER_VAR < PR_OUTER_SIZE && !isnan(abl.measured_z); PR_OUTER_VAR++) {
        ...
        for (PR_INNER_VAR = inStart; PR_INNER_VAR != inStop; pt_index++, PR_INNER_VAR += inInc) {
          ...
          if (isnan(abl.measured_z)) {
            ...
            {
              temp_comp.compensate_measurement(TSI_PROBE_WINTER, thermalManager.degProbe(), abl.measured_z);
            }
            else
            {
            ...
            if(isnan(added_z_value[abl.meshCount.x][abl.meshCount.y])) added_z_value[abl.meshCount.x][abl.meshCount.y] = 0; //cremaker

            const float z = abl.measured_z + abl.Z_offset;
            z_values[abl.meshCount.x][abl.meshCount.y] = z + added_z_value[abl.meshCount.x][abl.meshCount.y]; //cremaker
